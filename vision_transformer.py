...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        # 添加一个线性层，用于将文本特征投影到与值矩阵 (v) 兼容的维度
#         self.text_proj = nn.Linear(512, head_dim)  # 将文本特征投影到 head_dim
        self.text_proj = nn.Linear(512, head_dim)  # 将文本特征投影到 head_dim
    ...
    def forward(self, x, text_features=None):
        ...
        if text_features is not None:
            # 检查文本特征的维度
#             print("text_features shape before projection:", text_features.shape)

            # 投影文本特征并确保其形状
            text_features = self.text_proj(text_features)  # [B, seq_length, head_dim]
#             print("text_features shape after projection:", text_features.shape)

            # 扩展文本特征维度以与v匹配
            text_features = text_features.unsqueeze(2)  # [B, seq_length, 1, head_dim]
            text_features = text_features.expand(-1, -1, self.num_heads, -1)  # [B, seq_length, num_heads, head_dim]
#             print("text_features shape after expansion:", text_features.shape)
            
            # 重新调整 v 的维度，使其与 text_features 匹配
            text_features = text_features.permute(0, 2, 1, 3)  # 调整 v 的维度为 [B, seq_len, num_heads, head_dim] 以匹配 text_features
#             print("text_features shape after expansion:", text_features.shape)
            
            # 确保 v 和 text_features 维度一致
#             print("v shape before adding text_features:", v.shape)
            v = v + text_features  # 加入文本特征到值矩阵 v 中
#             print("v shape after adding text_features:", v.shape)
...
class Block(nn.Module):
    ...
    def forward(self, x,text_features=None):
        # 自注意力层：如果有文本特征，传递给自注意力计算
        x = x + self.drop_path(self.attn(self.norm1(x),text_features=text_features))
        # 前馈网络
...
#                  drop_path_rate=0., hybrid_backbone=None, norm_layer=nn.LayerNorm,text_encoder=None):
    def __init__(self, img_size=32, patch_size=4, in_chans=3, num_classes=100, embed_dim=128, depth=5,
                 num_heads=2, mlp_ratio=4., qkv_bias=True, qk_scale=None, drop_rate=0., attn_drop_rate=0.,
                 drop_path_rate=0., hybrid_backbone=None, norm_layer=nn.LayerNorm,text_encoder=None):
        super().__init__()
    ...
    def forward_features(self, x, text_features=None):
        ...
        for blk in self.blocks:
            x = blk(x)
        # 将视觉特征进行归一化，得到 xdecode
        x = self.norm(x)
        
        # 如果有文本特征，进行第二次前向传播
        if text_features is not None:
            for blk in self.blocks:
                x = blk(x, text_features=text_features)
        
        return x[:, 0]

    def forward(self, x, text_features=None):
        x = self.forward_features(x,text_features)
        x = self.head(x)
        return x
