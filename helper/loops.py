...
def train_vanilla(epoch, train_loader, model, criterion, optimizer, opt,return_feat=False):
    """vanilla training"""
    ...
    for idx, batch_data in enumerate(train_loader):
        ...
        # output = model(input)
        if return_feat:
            _, output = model(input, return_feat=return_feat)
        else:
            output = model(input)
...
def validate(val_loader, model, criterion, opt,return_feat=False):
    """validation"""
...
def train_distill_multi_teacher(epoch, train_loader, module_list, mlp_net, cos_value, criterion_list, optimizer, opt):
    ...
    # 先将model_t_list保存至model_t_list_all
    # model_t_list_all = model_t_list[:]
    for idx, data in enumerate(train_loader):
        ...
        with torch.no_grad():
            for model_t in model_t_list:
                ...
                # 先全部放进去
                # tea_loss_cls = criterion_cls(logit_t, target)
                # tea_loss_cls = criterion_cls(logit_t, logit_s)
                # tea_loss_cls_list.append(tea_loss_cls)
                feat_t_list.append(feat_t)
                logit_t_list.append(logit_t)
        # 查看哪个tea_loss_cls最高就舍弃哪个
        # max_value = max(tea_loss_cls_list)
        # max_index = tea_loss_cls_list.index(max_value)
        # print(tea_loss_cls_list,max_value,max_index)
        # 三个用于计算损失的数组都要嘎掉
        # del tea_loss_cls_list[max_index]
        # del feat_t_list[max_index]
        # del logit_t_list[max_index]
        # del model_t_list[max_index]

        # 计算温度
        if opt.have_mlp:
            temp = mlp_net(None, logit_s, cos_value)
            # print('--------temp-------',temp)
            # print('+++++++',temp)
            temp = opt.t_start + opt.t_end * torch.sigmoid(temp)
            temp = temp.cuda()  # 包含单个元素的一维张量
        else:
        ...
        #     loss_t_list = [criterion_cls_lc(logit_t, target) for logit_t in logit_t_list] # logit_s
        ...
        #     # loss_div_list = [criterion_div(logit_s, logit_t, is_ca=True)
        #     #                  for logit_t in logit_t_list]
        #     loss_div_list = [criterion_div(logit_s, logit_t, is_ca=True, temp=temp)
        #                      for c_i, logit_t in enumerate(logit_t_list)]
        #
        #     loss_div = torch.stack(loss_div_list, dim=0)
        #     bsz = loss_div.shape[1] / 2
        #     loss_div = (torch.mul(attention, loss_div).sum()) / (1.0*bsz)
        #     # print('--------loss--------',temp , loss_div)
        elif opt.ensemble_method == 'CAMKD':
            # 目标的attention
            criterion_cls_lc = nn.CrossEntropyLoss(reduction='none')
            ...
            # 学生logit的attention
            # criterion_cls_lc = nn.CrossEntropyLoss(reduction='none')
            # loss_t_list = [criterion_cls_lc(logit_t, logit_s) for logit_t in logit_t_list]
            # loss_t = torch.stack(loss_t_list, dim=0)
            # attention = (1.0 - F.softmax(loss_t, dim=0)) / (opt.teacher_num - 1)
            ...
            # 平均attention
            # attention = torch.tensor(1 / 3).cuda()
            # attention = attention.expand(loss_div.shape)
        ...
        if opt.distill == 'kd':
            ...
            # 以学生输出为导向
            # loss_kd, weight = criterion_kd(trans_feat_s_list, mid_feat_t_list, output_feat_t_list, logit_s)
            # 以平均值为权重
            loss_kd,weight = criterion_kd(trans_feat_s_list, mid_feat_t_list, output_feat_t_list, target)
